...
export default function ProfessorListagem() {
	const [paramsFilter, setParamsFilter] = useState<FiltrosPessoa>({tipo: TipoPessoa.professor} as FiltrosPessoa);
			...
			<main>
				...
				<div className="h-screen bg-gray-50 flex">
                	...
                	className="flex py-1 px-2 rounded-md items-center text-green-600 border-[1px] border-green-200 bg-green-100  hover:text-green-800"
							>
                	<Link href={{pathname: "/pessoa", query: {tipo: TipoPessoa.professor}}}  className='w-full flex flex-row select-none items-center'>
						...
						<div className="w-full grid grid-cols-4 flex-col py-4">
							{/* Input text to filter table based on name */}
							...
							<div className="col-span-4">
								<TablePessoa params={paramsFilter} currentUser={user} getId={(pessoa: Pessoa) => pessoa.professorId} type="professor" />
							</div>
						</div>
					</div>
				</div>
