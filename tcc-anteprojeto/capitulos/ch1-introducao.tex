...
% Ref 1
% Conforme mencionado anteriormente, \citeonline{duarte:2014} foi o primeiro a experimentar o método FrameWeb no processo de desenvolvimento de um sistema chamado SCAP. Foi utilizado Java EE como a plataforma de desenvolvimento, incluindo seu conjunto de \textit{frameworks} padrão. Sucessivamente outros trabalhos expandiram a utilização de novos \textit{frameworks} e contribuíram para a fortalecimento e analise do método. Por exemplo, \citeonline{guterres:2019} desenvolveu o SCAP utilizando o \textit{framework} Play, \citeonline{berger:2021} com os \textit{frameworks} Yii e Symfony, \citeonline{dalapicola:2021} com os \textit{frameworks} Quasar e AdonisJS, dentre outros.\footnote{Outros trabalhos de conclusão de curso que implementaram o SCAP com diferentes \textit{frameworks} e realizando sua modelagem com o FrameWeb podem ser encontrados em \url{http://www.inf.ufes.br/~vitorsouza/orientacao/trabalhos-orientados/}.}
% Neste mesmo espírito, o presente trabalho tem como motivação utilizar o Angular, que é um \textit{framework} que vem sendo amplamente utilizado no mercado atual, seguindo o método FrameWeb~\cite{souza:2007,souza-celebratingfalbo20}. Com a finalidade de contribuir para a avaliação do método com um novo conjunto de \textit{frameworks}.
% ------
% Ref 2
% Como visto, o processo de requisição de afastamento é longo e cheio de detalhes, portanto a possibilidade de um sistema Web para auxiliar o DI é bastante atraente. Tal proposta de um ambiente de fácil usabilidade e com um mecanismo de notificações automáticas beneficia não apenas os secretários do DI, como também os professores que requisitam ou avaliam afastamentos,   facilitando o trabalho para todos.
% Além de ser um tema relevante dentro do contexto do DI, o desenvolvimento do sistema, com o método FrameWeb, apresenta uma vasta gama de possibilidade de estudo dentro da área de computação e desenvolvimento Web, proporcionando um grande aprendizado durante o processo de sua implementação. 
% Por fim, esse projeto também representa um passo a mais no desenvolvimento do método FrameWeb, oferecendo uma oportunidade de analisar como sua aplicação se encaixa no contexto de um \textit{framework} diferente, o Play, e comparar com implementações anteriores do SCAP em outros frameworks.

% ------

% Motivação e justificativa baseadas nas referências a cima
